	...
	if (error instanceof TimeoutError) {
		const errorDetails: McpErrorResponse['error'] = {
			...
			guidance: [
				...
				'Avoid deep `offset` paging — large offsets get progressively slower until they time out. For `searchSymbols`, narrow with `filterByKind`, `isExported`, or a more specific `query`; for graph methods, reduce `depth` instead of paginating deeper.',
