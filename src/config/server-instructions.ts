...
Decision rule: When you consider using Grep/Glob calls to understand code structure, use \`query_code\` instead—it answers structural questions in one call.

Use \`query_code\` for: symbols, definitions, dependencies, usage, impact, architecture
...
5. **Errors are structured** — Failed queries return \`{error: {code, message, guidance[]}}\`, not exceptions. Empty results return empty arrays, not errors.
