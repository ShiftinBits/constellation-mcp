	...
	protected formatResult(
		...
		if (resultCount === 0) {
			...
			if (params?.filterByKind && params.filterByKind.length > 0) {
				output += `- Remove or broaden filterByKind (currently: ${params.filterByKind.join(', ')})\n`;
			}
			if (params?.filePattern) {
				output += `- Remove or broaden filePattern (currently: "${params.filePattern}")\n`;
			}
			...
			output += `- **get_symbol_details** - Get full details for specific symbols (use symbolId from results)\n`;
			if (symbols.some(s => s.usageCount !== undefined && s.usageCount > 0)) {
				output += `- **trace_symbol_usage** - See where heavily-used symbols are called\n`;
			}
			if (symbols.some(s => s.isExported)) {
				output += `- **get_dependents** - Find what depends on exported symbols\n`;
			}
		} else if (resultCount <= 50) {
			output += `Found ${totalCount} symbol${totalCount === 1 ? '' : 's'}${hasMore ? ` (showing ${resultCount})` : ''}.\n\n`;
			if (hasMore && pagination?.nextOffset !== undefined) {
				output += `- Use **offset** parameter to see more results (next page: offset=${pagination.nextOffset})\n`;
			}
			output += `- **get_symbol_details** - Examine specific symbols by symbolId\n`;
			if (!params?.filterByKind || params.filterByKind.length === 0) {
				output += `- **Narrow results** - Use filterByKind to focus on specific types (function, class, etc.)\n`;
			}
			if (!params?.filePattern) {
			...
			if (!params?.filePattern) {
				output += `- Add **filePattern** - Limit to specific directories (e.g., "src/utils/**")\n`;
			}
			if (params?.isExported === undefined) {
				output += `- Add **isExported=true** - Show only public API symbols\n`;
			}
			if (hasMore && pagination) {
				output += `\n**Pagination:**\n`;
				output += `- Current offset: ${pagination.currentOffset}\n`;
				if (pagination.nextOffset !== undefined) {
					output += `- Next page: offset=${pagination.nextOffset}\n`;
				}
			}
		}
